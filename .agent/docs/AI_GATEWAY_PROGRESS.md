...
| MyBatis-Plus 3.5.16 | ORM 层，6 个业务 Mapper 接口均继承 `BaseMapper` |
...
| `ai_channel` | 自定义渠道，整合供应商类型、baseUrl、请求路径、模型列表路径、上游 API Key 以及 AUTH 类型渠道的 auth.json 文件引用 |
...
### 2.1 AUTH 文件存储（V12 新增）

- `ai_channel` 新增 `auth_mode`、`auth_file_path`、`auth_status`、`auth_subject`、`auth_expires_at` 字段。
- SQLite 默认把授权文件保存到数据库文件父目录同级的 `auth-dir`；MySQL 默认保存到 `/opt/data/auth-dir`。
- 可通过 `api-convert.auth.storage-dir` 或 `API_CONVERT_AUTH_STORAGE_DIR` 覆盖授权文件根目录。
- 授权文件按 `{providerType}/{channelCode}.json` 隔离保存，管理端响应只返回脱敏身份、状态和过期时间，不返回 access token 或 refresh token。
...
| 组件 | 说明 |
|---|---|
| `ProviderType` 枚举 | `OPENAI_COMPATIBLE`, `ANTHROPIC`, `OPENAI_RESPONSES`, `GPT_AUTH`, `CLAUDE_AUTH`, `GEMINI`, `DEEPSEEK_CHAT`, `DEEPSEEK_ANTHROPIC`, `LOCAL` |
...
| `GptAuthProviderClient` | GPT AUTH Client，复用 OpenAI Chat 兼容链路，从 auth.json 读取 Bearer access token |
| `ClaudeAuthProviderClient` | Claude AUTH Client，复用 Anthropic Messages 链路，从 auth.json 读取 Bearer access token |
...
- `ModelRoute` — 路由解析结果（publicModel, providerCode, providerType, providerModel, baseUrl, chatPath, apiKey, authMode, authFilePath）
...
| CHAT_COMPLETIONS → DEEPSEEK_CHAT | `ChatCompletionsToDeepSeekChatAdapter` | 复用 Chat Completions 风格上游语义，保留 DeepSeek Chat thinking 续轮所需字段边界 |
| CHAT_COMPLETIONS → DEEPSEEK_ANTHROPIC | `ChatCompletionsToDeepSeekAnthropicAdapter` | OpenAI Chat 入口转 DeepSeek Anthropic 风格上游，隔离 DeepSeek thinking 内容块兼容逻辑 |
...
| ANTHROPIC_MESSAGES → DEEPSEEK_CHAT | `AnthropicToDeepSeekChatAdapter` | Anthropic Messages 入口转 DeepSeek Chat 上游，复用工具消息转换并适配 DeepSeek Chat 专属字段 |
| ANTHROPIC_MESSAGES → DEEPSEEK_ANTHROPIC | `AnthropicMessagesToDeepSeekAnthropicAdapter` | Anthropic Messages 入口转 DeepSeek Anthropic 上游，保持 thinking 块处理限定在独立供应商路径 |
...
| OPENAI_RESPONSES → DEEPSEEK_CHAT | `ResponsesToDeepSeekChatAdapter` | Responses 历史中的 `reasoning` item 恢复为 DeepSeek Chat 续轮需要的 `reasoning_content`，并兜底历史 assistant 消息字段 |
| OPENAI_RESPONSES → DEEPSEEK_ANTHROPIC | `ResponsesToDeepSeekAnthropicAdapter` | Responses 入口转 DeepSeek Anthropic 上游，复用 Anthropic Messages 结构并隔离 DeepSeek thinking 兼容行为 |
...
## 当前包结构（共 151 个 Java 源文件 + 9 个测试文件）
...
│       ├── AnthropicMessagesToDeepSeekAnthropicAdapter.java # ANTHROPIC → DEEPSEEK_ANTHROPIC
│       ├── AnthropicMessagesToGeminiAdapter.java          # ANTHROPIC → GEMINI（1.0.2 新增）
│       ├── AnthropicTools.java                            # Anthropic 工具结构转换辅助
│       ├── AnthropicToDeepSeekChatAdapter.java            # ANTHROPIC → DEEPSEEK_CHAT
...
│       ├── ChatCompletionsToDeepSeekAnthropicAdapter.java # CHAT_COMPLETIONS → DEEPSEEK_ANTHROPIC
│       ├── ChatCompletionsToDeepSeekChatAdapter.java      # CHAT_COMPLETIONS → DEEPSEEK_CHAT
...
│       ├── ResponsesToDeepSeekAnthropicAdapter.java       # OPENAI_RESPONSES → DEEPSEEK_ANTHROPIC
│       ├── ResponsesToDeepSeekChatAdapter.java            # OPENAI_RESPONSES → DEEPSEEK_CHAT
...
- Channel management now exposes `GPT_AUTH` and `CLAUDE_AUTH`, hides API Key/Base URL/request path/model-list path inputs for AUTH channels, and provides `auth.json` upload plus OAuth authorization link generation with open/copy actions and manual callback URL submission.
...
- Added V12 auth-file provider support for `GPT_AUTH` and `CLAUDE_AUTH`: admin channels can upload `auth.json`, store it under `auth-dir` by provider type, and route using the saved Bearer access token without exposing token contents in API responses.
- `GPT_AUTH` and `CLAUDE_AUTH` channels now auto-fill official default upstream addresses and paths when saved: `https://api.openai.com` + `/v1/chat/completions`, and `https://api.anthropic.com` + `/v1/messages`.
- Added OAuth start/callback endpoints for `GPT_AUTH` and `CLAUDE_AUTH`; default Codex/OpenAI and Claude OAuth metadata is built in, while explicit configuration can still override it. Admins can paste the full localhost callback URL into `POST /api/admin/channels/{id}/auth/callback-url`.
- `ChatGatewayService` now maps `GPT_AUTH` to OpenAI-compatible adapters and `CLAUDE_AUTH` to Anthropic adapters so cross-protocol request/response conversion continues to work for auth-file channels.
