...
/**
 * 图片生成网关服务，复用 API Key、模型路由和请求日志能力，向支持图片的供应商透传请求。
 */
@Service
public class ImageGatewayService {

    private static final Logger log = LoggerFactory.getLogger(ImageGatewayService.class);
    private static final String SOURCE_PROTOCOL = "openai";
    private static final String REQUEST_TYPE = "images";
    ...
    /**
     * 执行 OpenAI Images API 生成调用；图片暂不按 token 计费，只记录请求数和审计日志。
     */
    public OpenAiImageResponse generate(OpenAiImageRequest request, HttpServletRequest servletRequest) {
    ...
    private void validate(OpenAiImageRequest request) {
        if (request == null) {
