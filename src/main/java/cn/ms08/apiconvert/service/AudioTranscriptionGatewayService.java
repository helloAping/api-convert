...
/**
 * 语音转写网关服务，复用 API Key、模型路由和请求日志能力，向 OpenAI 兼容供应商透传 multipart 上传；
 * 上游按 verbose_json 返回时不按 token 计费（音频时长单价），只记请求数和审计日志。
 */
@Service
public class AudioTranscriptionGatewayService {

    private static final Logger log = LoggerFactory.getLogger(AudioTranscriptionGatewayService.class);
    private static final String SOURCE_PROTOCOL = "openai";
    private static final String REQUEST_TYPE = "audio_transcriptions";
    ...
    public AudioTranscriptionGatewayService(RoutingService routingService, ProviderClientRegistry providerClientRegistry,
    ...
    /**
     * 执行 OpenAI Audio Transcriptions API 调用；multipart 文件直传到上游。
     */
    public OpenAiAudioTranscriptionResponse transcribe(OpenAiAudioTranscriptionRequest request,
        ...
        try {
            ...
            route = routingService.resolveModel(request.getModel(), principal.apiKeyId(),
                    principal.allowedChannelCodes(), principal.allowedModelNames(), EndpointType.AUDIO_TRANSCRIPTIONS);
            ...
            try {
                OpenAiAudioTranscriptionResponse response = providerClientRegistry.get(route.providerType())
                        .transcribe(route, request);
                ...
                usageRecorder.recordSuccess(requestId, principal.apiKeyId(), SOURCE_PROTOCOL, REQUEST_TYPE,
                        EndpointType.AUDIO_TRANSCRIPTIONS.name(), EndpointType.AUDIO_TRANSCRIPTIONS.name(),
                        route, false, HttpStatus.OK.value(), System.currentTimeMillis() - start, null);
                return response;
            } finally {
                metrics.recordUpstream(EndpointType.AUDIO_TRANSCRIPTIONS, route.providerType().name(),
                        upstreamStatus, System.nanoTime() - upstreamStart);
            }
        } catch (ProviderException exception) {
            routingService.recordFailure(principal.apiKeyId(), null, route, null);
            metrics.recordError(EndpointType.AUDIO_TRANSCRIPTIONS, exception.code().name());
            log.warn("语音转写上游调用失败：model={} channel={} error={}",
            ...
            metrics.recordError(EndpointType.AUDIO_TRANSCRIPTIONS, ErrorCode.INTERNAL_ERROR.name());
            log.error("语音转写网关异常：{}", exception.getMessage(), exception);
    ...
    private void validate(OpenAiAudioTranscriptionRequest request) {
        if (request == null) {
        ...
        if (request.getFileBytes() == null || request.getFileBytes().length == 0) {
            throw new GatewayException(ErrorCode.INVALID_REQUEST, HttpStatus.BAD_REQUEST, "file is required");
        }
    }
